        ...
        try {
            if (!data.roomId) {
             ...
             for(let i =0 ; i< message.length; i++){
                message[i].userData = await db.User.findOne({where:{id:message[i].userId},attributes: {
                        exclude: ['userId']
                    }})
        ...
        try {
            if (!userId) {
             ...
             for(let i =0 ; i< room.length; i++){
                ...
                room[i].userOneData = await db.User.findOne({where:{id:room[i].userOne},attributes: {
                        exclude: ['userId']
                    } })
                if(room[i].userOneData.image){
                    room[i].userOneData.image = new Buffer(room[i].userOneData.image, 'base64').toString('binary');
                }
                room[i].userTwoData = await db.User.findOne({where:{id:room[i].userTwo},attributes: {
                        exclude: ['userId']
                    }})
...
let listRoomOfAdmin = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let user = await db.User.findOne({where:{email:'chat@gmail.com'},attributes: {
                    exclude: ['userId']
                }})
            if(user){
                let room = await db.RoomMessage.findAll({
